...
function setupSmartTask() {
  ...
  setupTasksFilter_();
  if (typeof refreshProjects_ === "function") refreshProjects_();
...
function setupListsSheet_() {
  ...
  sheet
    .getRange(1, 1, 1, listNames.length)
    .setValues([listNames])
    .setFontWeight("bold")
  ...
  const maxLen = listNames.reduce(function (m, name) {
    return Math.max(m, LOOKUP_LISTS[name].length);
  }, 0);
  const grid = [];
  for (let r = 0; r < maxLen; r++) {
    grid.push(
      listNames.map(function (name) {
        return LOOKUP_LISTS[name][r] || "";
      }),
    );
  }
  if (grid.length > 0) {
...
function setupSettingsSheet_() {
  ...
  if (sheet.getLastRow() < 2) {
    sheet
      .getRange(2, 1, DEFAULT_SETTINGS.length, 3)
      .setValues(DEFAULT_SETTINGS);
  } else {
    const existing = readTable_(SHEETS.SETTINGS, headers).map(function (r) {
      return r.Key;
    });
    const missing = DEFAULT_SETTINGS.filter(function (row) {
...
function setupTaskValidation_() {
  ...
  applyListValidation_(
    sheet,
    TASK_HEADERS.indexOf("Area") + 1,
    maxRows,
    LOOKUP_LISTS.Area,
  );
  applyListValidation_(
    sheet,
    TASK_HEADERS.indexOf("Priority") + 1,
    maxRows,
    LOOKUP_LISTS.Priority,
  );
  applyListValidation_(
    sheet,
    TASK_HEADERS.indexOf("Status") + 1,
    maxRows,
    LOOKUP_LISTS.Status,
  );
  applyListValidation_(
    sheet,
    TASK_HEADERS.indexOf("Energy") + 1,
    maxRows,
    LOOKUP_LISTS.Energy,
  );
  applyListValidation_(
    sheet,
    TASK_HEADERS.indexOf("Context") + 1,
    maxRows,
    LOOKUP_LISTS.Context,
  );
  applyListValidation_(
    sheet,
    TASK_HEADERS.indexOf("RecurringType") + 1,
    maxRows,
    LOOKUP_LISTS.RecurringType,
  );
  ...
  const progressRule = SpreadsheetApp.newDataValidation()
    ...
    .setHelpText("Progress must be a number from 0 to 100.")
    .build();
...
function setupProjectValidation_() {
  ...
  applyListValidation_(
    sheet,
    PROJECT_HEADERS.indexOf("Area") + 1,
    maxRows,
    LOOKUP_LISTS.Area,
  );
  applyListValidation_(
    sheet,
    PROJECT_HEADERS.indexOf("Health") + 1,
    maxRows,
    LOOKUP_LISTS.ProjectHealth,
  );
}
...
function setupGoalValidation_() {
  ...
  applyListValidation_(
    sheet,
    GOAL_HEADERS.indexOf("Area") + 1,
    maxRows,
    LOOKUP_LISTS.Area,
  );
  applyListValidation_(
    sheet,
    GOAL_HEADERS.indexOf("Status") + 1,
    maxRows,
    LOOKUP_LISTS.GoalStatus,
  );
}
...
function setupHabitValidation_() {
  ...
  applyListValidation_(
    sheet,
    HABIT_HEADERS.indexOf("Frequency") + 1,
    maxRows,
    LOOKUP_LISTS.HabitFrequency,
  );
}
...
function setupTaskFormatting_() {
  ...
  const riskRange = sheet.getRange(
    2,
    TASK_HEADERS.indexOf("Risk") + 1,
    lastRow,
    1,
  );
  const dueDateRange = sheet.getRange(
    2,
    TASK_HEADERS.indexOf("DueDate") + 1,
    lastRow,
    1,
  );
  ...
  Object.keys(statusColors).forEach(function (val) {
    rules.push(
      SpreadsheetApp.newConditionalFormatRule()
        .whenTextEqualTo(val)
        .setBackground(statusColors[val][0])
        .setFontColor(statusColors[val][1])
  ...
  Object.keys(priorityColors).forEach(function (val) {
    rules.push(
      SpreadsheetApp.newConditionalFormatRule()
        .whenTextEqualTo(val)
        .setBackground(priorityColors[val][0])
        .setFontColor(priorityColors[val][1])
  ...
  Object.keys(riskColors).forEach(function (val) {
    rules.push(
      SpreadsheetApp.newConditionalFormatRule()
        .whenTextEqualTo(val)
        .setBackground(riskColors[val][0])
        .setFontColor(riskColors[val][1])
        .setRanges([riskRange])
        .build(),
    );
  });
